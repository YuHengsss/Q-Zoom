...
def collect_tp_stats(tp_row):
    pkl_dir_str = tp_row.get("pkl_dir", "")
    pkl_dir = Path(pkl_dir_str) if pkl_dir_str else None

    # Backward-compatible fallback: infer pkl_dir from results_file sibling.
    if pkl_dir is None or not str(pkl_dir):
        rf = tp_row.get("results_file", "")
        if rf:
            pkl_guess = Path(rf).parent / "pkls"
            if pkl_guess.exists():
                pkl_dir = pkl_guess
