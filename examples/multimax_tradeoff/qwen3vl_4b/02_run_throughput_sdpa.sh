...
OUTPUT_ROOT="${OUTPUT_ROOT:-${CODE_ROOT}/logs/qwen3vl_4b_multimax_tradeoff/throughput}"
...
run_one() {
  ...
  if [[ "${max_tokens}" == "256" ]]; then min_t=128; fi
  local group
  ...
  if [[ -n "${existing}" ]]; then
    ...
    echo "[TradeoffTP] run ${variant} ${setting_id} ${task} samples=${SAMPLE_LIMIT}"
