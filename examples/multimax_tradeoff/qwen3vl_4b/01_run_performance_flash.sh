...
# for Qwen3-VL-4B baseline vs Q-Zoom (Stage 3).
#
# Usage:
...
#
# Required env: conda env qzoom-q3 (transformers==4.57.0) — see ../../../README.md.
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${CODE_ROOT}/logs/qwen3vl_4b_multimax_tradeoff/performance}"
...
BASELINE_CKPT="${BASELINE_CKPT:-Qwen/Qwen3-VL-4B-Instruct}"
MODEL_ROOT="${MODEL_ROOT:-${CODE_ROOT}/output}"
STAGE3_CKPT="${STAGE3_CKPT:-${MODEL_ROOT}/qzoom-qwen3vl-4b}"

MODEL_TYPE="${MODEL_TYPE:-qwen3_vl}"
# Qwen3-VL: patch_size=32 (vs Qwen2.5-VL's 28)
PATCH_SIZE=32
...
get_stage_thresholds() {
  ...
  case "${task}" in
    textvqa_val|infovqa_val|docvqa_val) roi="0.15" ;;
    chartqa)                            roi="0.15" ;;
    ocrbench)                           roi="0.125" ;;
    vstar_bench)                        roi="0.10" ;;
    mmerealworld_lite)                  roi="0.05" ;;
    hrbench)                            roi="0.10" ;;
    *) roi="0.10" ;;
  esac

  case "${task}" in
    vstar_bench|mmerealworld_lite|hrbench) high="0.05" ;;
    *) high="0.10" ;;
  esac
  echo "${roi} ${high} ${two_stage}"
}
...
run_one() {
  ...
  # When max_tokens >= 2048 and not baseline, use min=512 for HR tasks only (Tab2 setting).
  # Doc/OCR tasks keep min=256 at all resolutions.
  local group
