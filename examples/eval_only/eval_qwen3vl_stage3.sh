#!/bin/bash
# Evaluate a Qwen3-VL Q-Zoom (Stage 3) checkpoint on the full
...
#     `high_res_thresh=0.10`, `min_tokens=256` (ocrbench is special: 128).
#   - HR     (max_tokens=4096): "Tab2" regime — `roi_conf_thresh=0.10`,
#     `high_res_thresh=0.025`, `min_tokens=512`.
...
LOG_SUFFIX_BASE="${LOG_SUFFIX_BASE:-qzoom_qwen3vl_eval}"
NUM_GPUS="${NUM_GPUS:-4}"
...
# ----------------------------------------------------------------------
run_eval textvqa_val  0.15  0.10 "${DOC_MIN_TOKENS}" "${DOC_MAX_TOKENS}"
run_eval infovqa_val  0.15  0.10 "${DOC_MIN_TOKENS}" "${DOC_MAX_TOKENS}"
run_eval chartqa      0.15  0.10 "${DOC_MIN_TOKENS}" "${DOC_MAX_TOKENS}"
run_eval ocrbench     0.125 0.10 "${OCR_MIN_TOKENS}" "${DOC_MAX_TOKENS}"
run_eval docvqa_val   0.15  0.10 "${DOC_MIN_TOKENS}" "${DOC_MAX_TOKENS}"

# ----------------------------------------------------------------------
# HR / Vision-centric (Tab2 regime, max_tokens=4096, min_tokens=512)
# ----------------------------------------------------------------------
run_eval vstar_bench       0.10 0.025 "${HR_MIN_TOKENS}" "${HR_MAX_TOKENS}"
run_eval mmerealworld_lite 0.10 0.025 "${HR_MIN_TOKENS}" "${HR_MAX_TOKENS}"
run_eval hrbench           0.10 0.025 "${HR_MIN_TOKENS}" "${HR_MAX_TOKENS}"
