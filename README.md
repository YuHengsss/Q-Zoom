...
**Stage 2 — LLM post-SFT (Targeted SFT).** Carry over the frozen TWIG
branch from Stage 1 and train **only the LLM decoder layers + `lm_head`**
on a hard-sample mixture mined by an LLM-as-a-Judge that compares the
direct response against the ROI response on a fresh question pool, so
the LLM adapts to operate under Q-Zoom gating.

<p align="center">
  <img src="assets/stage2_targeted_sft.png" alt="Stage 2 — Targeted Post-SFT mined via LLM-as-a-Judge" width="50%">
</p>
...
**Stage 3 — High-resolution gating refinement (Dynamic Gate).** Train
**only the high-res gating network** (`high_res_layers` + `high_res_head`),
which decides whether each input needs the high-resolution ROI refinement
pass. The base LLM, `lm_head`, and TWIG branch are all frozen. Stage 3
uses a *consistency-aware* training sample generation pipeline that filters
unstable / contradictory examples before BCE supervision.

<p align="center">
  <img src="assets/stage3_gated_roi.png" alt="Stage 3 — Dynamic Gating Network with consistency-aware sample filtering" width="55%">
</p>
