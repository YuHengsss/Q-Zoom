...
def register_hooks(model, is_qwen3vl=False):
    """Register hooks at all stages of processing."""
    activations = {}
    hooks = []
    layers = model.model.layers if not is_qwen3vl else model.model.language_model.layers
...
def plot_attention_across_layers(attn_weights, layers=32, save_path=None, title_prefix="", avg_heads=True):
    ...
    if False:
        # 2. HEATMAP VISUALIZATION FOR LAYER-WISE ATTENTION
        # Prepare normalized data for heatmap
        heatmap_data = np.zeros((4, len(valid_layers)))
        for i, layer_idx in enumerate(valid_layers):
            for j, segment in enumerate(segments):
                heatmap_data[j, i] = segment_avgs[segment][i]

        # Normalize by row (segment) for better visualization
        row_sums = heatmap_data.sum(axis=1, keepdims=True)
        row_sums[row_sums == 0] = 1  # Avoid division by zero
        norm_heatmap = heatmap_data / row_sums

        im = ax2.imshow(norm_heatmap, cmap=cmap, aspect='auto', vmin=0, vmax=np.max(norm_heatmap) * 1.1)

        # Add text annotations
        for i in range(4):
            for j in range(len(valid_layers)):
                text_color = 'white' if norm_heatmap[i, j] > 0.5 else 'black'
                ax2.text(j, i, f'{heatmap_data[i, j]:.3f}',
                         ha='center', va='center', color=text_color, fontsize=9)

        # Set labels
        ax2.set_yticks(range(4))
        ax2.set_yticklabels(segments, fontsize=12)
        ax2.set_xticks(range(len(valid_layers)))
        ax2.set_xticklabels(valid_layers, fontsize=10)
        ax2.set_xlabel('Layer', fontsize=14)
        ax2.set_title(f'{title_prefix}Attention Heatmap Across Layers', fontsize=16)

        # Add colorbar
        cbar = fig2.colorbar(im, ax=ax2)
        cbar.set_label('Normalized Attention Weight', fontsize=12)

        # 3. PER-HEAD ANALYSIS FOR SELECTED LAYERS
        # Select a few representative layers
        num_layers_to_show = min(6, len(valid_layers))
        selected_layers = valid_layers[::max(1, len(valid_layers) // num_layers_to_show)][:num_layers_to_show]

        fig3, axes = plt.subplots(1, num_layers_to_show, figsize=(16, 6), sharey=True)
        if num_layers_to_show == 1:
            axes = [axes]

        # For each selected layer, show attention distribution across heads
        for i, layer_idx in enumerate(selected_layers):
            layer_key = f'layer{layer_idx}'

            # Accumulate data across generation steps
            head_data = {segment: [] for segment in segments}

            for segment, key in zip(segments, segment_keys):
                if (attn_weights[layer_key][key] and len(attn_weights[layer_key][key]) > 0):
                    # Stack data from all generation steps
                    stacked = np.vstack([weights for weights in attn_weights[layer_key][key]])
                    # Average across generation steps for each head
                    head_data[segment] = np.mean(stacked, axis=0)

            # Create a stacked bar chart for heads
            bottom = np.zeros(num_heads)

            for segment, color in zip(segments, segment_colors):
                if len(head_data[segment]) > 0:
                    axes[i].bar(range(num_heads), head_data[segment], bottom=bottom,
                                label=segment if i == 0 else "", color=color, alpha=0.8)
                    bottom += head_data[segment]

            axes[i].set_title(f'Layer {layer_idx}', fontsize=14)
            axes[i].set_xlabel('Head', fontsize=12)
            axes[i].set_xticks(range(num_heads))
            axes[i].set_xticklabels([f'{h}' for h in range(num_heads)], rotation=90)
        ...
        fig3.legend(handles, labels, loc='upper center', ncol=4, fontsize=12, bbox_to_anchor=(0.5, 0.98))
        fig3.tight_layout()
        fig3.subplots_adjust(top=0.85)
        fig3.suptitle(f'{title_prefix}Head-Level Attention Distribution', fontsize=16, y=0.99)
...
def plot_image_with_heatmaps(image, attention_maps, save_path=None, figsize=None, dpi=100):
    """
    Plots an image alongside one or more of its attention maps (heatmaps) in a smart grid layout.
    ...
    """
    # --- 1. Set up the figure and smart grid ---
    num_maps = len(attention_maps)
    if num_maps == 0:
        print("Warning: No attention maps provided to plot.")
        return

    total_plots = num_maps + 1  # Original image + attention maps

    # Calculate rows and columns for a roughly square grid
    ncol = int(math.ceil(math.sqrt(total_plots)))
    nrow = int(math.ceil(total_plots / ncol))

    if figsize is None:
        # Scale figsize based on grid size to keep plots readable
        figsize = (4 * ncol, 4 * nrow)

    fig, axes = plt.subplots(nrow, ncol, figsize=figsize, dpi=dpi)

    # Flatten the axes array to handle 1D or 2D grids uniformly
    if isinstance(axes, np.ndarray):
        axes_flat = axes.flatten()
    else:
        axes_flat = [axes]  # Handle single subplot case
    ...
    if isinstance(image, str):
        ...
        # Ensure UnNormalize is available in your scope
        unnorm = UnNormalize([0.48145466, 0.4578275, 0.40821073], [0.26862954, 0.26130258, 0.27577711])
    ...
    # Plot original image at index 0
    axes_flat[0].imshow(img)
    axes_flat[0].axis('off')
    # axes_flat[0].set_title("Original") # Optional

    # --- 3. Loop through and plot each attention map ---
    for i, map_info in enumerate(attention_maps):
        ax = axes_flat[i + 1]  # +1 because index 0 is the original image

        raw_attn_map = map_info['map']
        # title = map_info.get('title', f'Map {i + 1}') # Optional title
        ...
        if blend_mask:
            ...
            # Avoid div by zero
            norm_map = raw_attn_map / max_val if max_val > 1e-6 else np.zeros_like(raw_attn_map)
    ...
    # If the grid is larger than the number of plots, hide the extra axes
